@echo off
REM Audio Transcription - ESTONIAN, Teams / video-call (variant 61)
...
echo ========================================
echo Audio Transcription - ESTONIAN (Teams)
echo ========================================
...
if /i "!STT_RUNTIME!"=="wsl" (
    wsl -e bash -c "export HF_HOME=\"$HOME/.cache/hf\" && export HF_HUB_CACHE=\"$HF_HOME/hub\" && cd !STT_WSL_REPO! && .venv/bin/python scripts/transcribe_manager.py process '!STT_AUDIO_DIR_WSL!' --language et --output-format txt --diarize --num-speakers 2 --variants '!VARIANTS_COMMA!'"
) else (
    set "HF_HOME=%USERPROFILE%\.cache\hf"
    set "HF_HUB_CACHE=%USERPROFILE%\.cache\hf\hub"
    pushd "!STT_REPO_WIN!"
    .venv\Scripts\python scripts\transcribe_manager.py process "!STT_AUDIO_DIR_RESOLVED!" --language et --output-format txt --diarize --num-speakers 2 --variants "!VARIANTS_COMMA!"
    popd
)
