...
if /i "!STT_RUNTIME!"=="wsl" (
    wsl -e bash -c "export HF_HOME=\"$HOME/.cache/hf\" && export HF_HUB_CACHE=\"$HF_HOME/hub\" && cd !STT_WSL_REPO! && .venv/bin/python scripts/transcribe_manager.py process '!STT_AUDIO_DIR_WSL!' --preset et-32 --language et --output-format txt !STT_DIARIZE_ARGS! --variants '!VARIANTS_COMMA!'"
) else (
    docker run --rm ^
      ...
      stt-faster:latest process /workspace --preset et-32 --language et --output-format txt !STT_DIARIZE_ARGS! --variants "!VARIANTS_COMMA!"
)
